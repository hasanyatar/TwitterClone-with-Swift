...
//  TwitterClone
//
//  Created by Hasan YATAR on 27.02.2023.
//
...
class ProfileTableViewHeader: UIView {
    ...
    private func configureButtons(){
        editProfileButton.addTarget(self, action:#selector(didTapFollowButton) , for: .touchUpInside)
        editProfileButton.addTarget(self, action:#selector(touchDownFollowButton) , for: .touchDown)
    }
    ...
    @objc private func touchDownFollowButton() {
        UIView.animate(withDuration: 0.2, delay: 0, options: .curveLinear ) { [weak self] in
            self?.editProfileButton.backgroundColor = .systemGray6
        }completion: { _ in }
    ...
    private func configureConstraints(){
        ...
        let followButtonConstraints = [
            ...
            //followButton.leadingAnchor.constraint(equalTo: profileAvatarImageView.trailingAnchor),
            editProfileButton.heightAnchor.constraint(equalToConstant: 40),
            editProfileButton.widthAnchor.constraint(equalToConstant: editProfileButton.frame.width + 25)
        ]
